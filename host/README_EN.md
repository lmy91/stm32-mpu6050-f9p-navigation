...
Caster-side MSM assembly time is no longer treated as serial queue wait. Each outgoing frame retains its received time and group-release time: assembly age is diagnostic only, while release-to-serial-write wait (including queue/credit/partial writes) remains bounded at two seconds. Dequeue and partial writes never refresh the send-wait timestamp. Total residence is diagnostic, not measurement correction age, and excludes transmission after the serial write.

The 1024-byte outstanding cap, ACK/progress deadlines, CRC and MSM capacity protections remain. MSM boundaries are determined only by `MMI=0`, a repeated constellation's new epoch, station changes and the 64 KiB cap; wall-clock network gaps never delete observations. Genuine local serial stalls still stop without replaying a possibly written prefix. The serial owner captures counters, pending bytes and timing immediately before stopping, rather than relying on a stale GUI snapshot. Regression covers WUH2 groups split across more than thirteen seconds and confirms that actual send stalls and missing ACKs still stop. Firmware, F9P configuration and CSV formats are unchanged.
