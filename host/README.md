...
界面显示网络重连次数、当前排队字节数/时间。串口到 GUI 的接收背压上限为 2 MiB（按**字节**精确计数，非 chunk 数）；GUI/记录长时间卡住、接收缓存逼近上限时会明确停止并报警（报错会给出 current/peak 字节数），不静默丢记录。CSV 解析保存仍在主线程，因此本程序是有缓冲和超时监测的实时采集工具，不承诺硬实时截止时间。
