...
已修复“caster 分段发送 MSM 被误判为串口排队”的问题。每条待发帧保留原始接收时刻和完整组释放时刻：MSM 组包时间只用于诊断，不再设置墙钟超时；从完整组释放到串口写入的等待（含网络待发队列、流控和部分写入）仍最多 2 秒。取出队列和部分写入不会刷新发送等待时间。总驻留只是诊断值，不是测量历元差分龄期，也不包含串口写入后的硬件传输时间。

1024 字节在途上限、STM32 ACK/转发无进展超时、CRC 校验、MSM 容量保护均保留，真实的本地堵塞仍停止下发且不重放可能已写出的字节。MSM 只按 `MMI=0`、同星座新历元、站号变化和 64 KiB 容量判断边界；网络墙钟间隔不会再删除观测。停止前由串口线程保存即时确认计数、待发字节、组包/发送/总驻留时间，避免只依赖旧的界面快照。此次修复不修改固件、F9P 配置或 CSV 格式。回归覆盖超过 13 秒的 WUH2 分段组包；真实发送超时与无 ACK 仍会停止。
